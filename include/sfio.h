...
#define __sf_putc(f, c) (_SF_(f)->_next >= _SF_(f)->_endw ? _sfflsbuf(_SF_(f), (int)((unsigned char)(c))) : (int)(*_SF_(f)->_next++ = (unsigned char)(c)))
#define __sf_getc(f) (_SF_(f)->_next >= _SF_(f)->_endr ? _sffilbuf(_SF_(f), 0) : (int)(*_SF_(f)->_next++))
...
#define sfstrseek(f, p, m)                                                                                                                           \
    ((m) == SEEK_SET   ? (((p) < 0 || (p) > (f)->_size) ? (char *)0 : (char *)((f)->_next = (f)->_data + (p)))                                       \
     ...
     : (m) == SEEK_END ? (((p) > 0 || (f)->_size + (p) < 0) ? (char *)0 : (char *)((f)->_next = (f)->_data + (f)->_size + (p)))                      \
                       : (char *)0)
