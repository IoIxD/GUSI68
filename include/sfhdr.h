...
#define SFMVUNSET(f) (!((f)->bits & SF_MVSIZE) ? 0 : (((f)->bits &= ~SF_MVSIZE), ((f)->size /= SF_NMAP)))
...
#define _SFOPEN(f) ((f)->mode == SF_READ ? _SFOPENRD(f) : (f)->mode == SF_WRITE ? _SFOPENWR(f) \
...
#define SFFROZEN(f) (((f)->mode & (SF_PUSH | SF_LOCK | SF_PEEK)) ? 1 : !((f)->mode & SF_STDIO) ? 0                \
